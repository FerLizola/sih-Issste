  ...
  public boolean estatus(){
       Connection miCon = (new Conexion_BD()).conexion();
        if(miCon!=null){
            try{
               Statement stmt = miCon.createStatement();
               String sql = "Update CitaMedica set Status = 'C' where RFC_Derechoh= '"+rfc+"';";
             stmt.executeUpdate(sql);
               miCon.close();
               return true;
   }catch(Exception e){
       JOptionPane.showMessageDialog(null, "El status de la cita no fue actualizado correctamente");
       return false;
   }
