     ...
     /* public boolean comprobarReubicacion(){
      ...
      public boolean modificarPersonal(){
        ...
        if(miCon!=null){
            try{
               Statement stmt = miCon.createStatement();
        String sql = "Update Personal set APaterno_Personal='"+APat+"', AMaterno_Personal= '"+
                AMat+"', Nombre_Personal= '"+nombre+"', Direccion_Personal = '"+
                Dir+"', CodigoPos_Personal = '"+CP+"', TelefonoCasa_Personal = '"+
                telca+"', TelefonoCelular_Personal = '"+telcel+"', CorreoElec_Personal = '"+
               correo+"', HorarioTrab_Personal = '"+Hora+"', Password_Personal = '"+
                pass+"', NomEsp_Personal= '"+especialidad+"',id_Clinica = '"+
                       clinica+"', id_AreaT = '"+areaT+"' where RFC_Personal = '"+RFC+"';";
               stmt.executeUpdate(sql);
                         miCon.close();
                        return true;//JOptionPane.showMessageDialog(null, "Actualizado");
            }catch(Exception e){
        return false;
//JOptionPane.showMessageDialog(null, "Error: "+e.getMessage(),"Error", JOptionPane.ERROR_MESSAGE);
            }
